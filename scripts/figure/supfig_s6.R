...
# Each plotted point is the coefficient of variation for one method/setting/metric.
...
pdf(file.path(out_dir, "Supplementary_Figure_S6_CV_recreated_final3.pdf"), width = 10.5, height = 6.2, useDingbats = FALSE)
par(mar = c(5.3, 7.0, 2.0, 1.2), mgp = c(3.0, 0.8, 0), family = "sans", las = 1,
    xaxs = "i", yaxs = "i")
plot.new()
usr <- c(0.5, 6.5, -3.1, 2.6)
plot.window(xlim = usr[1:2], ylim = usr[3:4])
line_width <- 1.15
axis(1, at = 1:6, labels = metric_order, cex.axis = 0.95, tck = -0.018, lwd = line_width, lwd.ticks = line_width)
axis(2, at = -3:2, labels = -3:2, cex.axis = 0.95, tck = -0.018, lwd = line_width, lwd.ticks = line_width)
box(lwd = line_width)
...
for (mi in seq_along(metric_order)) {
  ...
  boxplot(vals_list, at = mi + unname(offsets[setting_order]), add = TRUE, width = rep(0.01, 3),
          axes = FALSE, outline = TRUE, outpch = 16, outcex = 0.70,
          outcol = adjustcolor("#4A4A4A", alpha.f = 0.55), col = unname(adjustcolor(cols[setting_order], alpha.f = 0.88)),
          border = "#4D4D4D",
          pars = list(boxlwd = line_width, medlwd = line_width,
                      whisklwd = line_width, staplelwd = line_width), staplewex = 0.55)
  for (si in seq_along(setting_order)) {
    ...
    if (length(vals) > 0) {
      ...
      points(jitter_x, vals, pch = 16, cex = 0.70, col = adjustcolor("#4A4A4A", alpha.f = 0.55))
    }
  }
}
